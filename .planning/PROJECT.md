...
**v1.6 — MCP 细节优化 (shipped 2026-04-01)**

- ✓ 端口号持久化：每个项目记住端口设置，重启后自动恢复
- ✓ 工具禁用状态持久化：Dock 面板工具开关重启后保留
- ✓ 端口冲突快速失败：去掉静默自增，push_error() 报错
- ✓ 工具调用超时：IO 线程 30s + game bridge 15s 超时机制
- ✓ 迟到响应丢弃：request ID 匹配，超时后不污染下一请求
- ✓ 双日志面板兼容：push_error() 替换 printerr()，Output + Debugger 同步
- ✓ 清理失效代码：TOOL_PARAM_HINTS 从 50 条精简到 30 条
...
- 30 MCP tools, 15 prompt templates, 5 MCP Resources (2 static + 3 URI templates)
- 28 phases shipped across 7 milestones (v1.0 → v1.1 → v1.2 → v1.3 → v1.4 → v1.5 → v1.6)
...
| ProjectSettings::save() 持久化端口和工具禁用 | 用户设置重启后丢失 | ✓ Good — 一行 save() 解决，去掉了 set_initial_value() |
| 去掉端口自增，改为 push_error() 快速失败 | 自增导致 bridge/plugin 端口不同步 | ✓ Good — 消除了隐性 bug |
| IO 线程 wait_for(30s) + request ID 追踪 | 无限等待导致 AI 客户端永远卡住 | ✓ Good — 迟到响应通过 ID 匹配安全丢弃 |
...
---
*Last updated: 2026-04-01 after v1.6 milestone complete*
