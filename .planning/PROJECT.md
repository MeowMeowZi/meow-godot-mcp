...
- [ ] 端口号持久化：每个项目记住自己的端口设置，重启后自动恢复
- [ ] 工具调用超时：IO 线程和 game bridge 延迟请求加超时机制，超时返回错误
- [ ] 双日志面板兼容：错误日志同时出现在 Output 和 Debugger 面板
- [ ] 工具禁用状态持久化：Dock 面板工具开关设置重启后保留
- [ ] 端口自增逻辑修复：去掉静默自增，避免插件和 bridge 端口不同步
- [ ] 清理失效代码：移除已删除工具的残留参数提示
...
- 30 MCP tools (v1.5 后精简合并), 15 prompt templates, 5 MCP Resources (2 static + 3 URI templates)
...
---
*Last updated: 2026-03-31 after v1.6 milestone started*
