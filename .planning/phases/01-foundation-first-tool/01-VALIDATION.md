...
- [ ] `tests/test_protocol.cpp` -- JSON-RPC parsing, initialize handshake, tools/list (with optional params), tools/call, error codes
- [ ] `tests/test_scene_tree.cpp` -- Scene tree JSON serialization format including transform, visible, has_script, script_path, depth truncation
...
| Addon directory structure valid | DIST-01 | Structural/packaging concern | Verify `addons/godot_mcp_meow/` contains .gdextension, bin/ |
